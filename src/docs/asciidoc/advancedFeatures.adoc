...
`"World"` into the `Context` under the key `"key"`.
<2> We `flatMap` on the source element, materializing the `Context` with `Mono.subscriberContext()`.
<3> We then use `map` to extract the data associated to `"key"` and concatenate that with
the original word.
...
StepVerifier.create(r)
            ...
            .verifyComplete();
----
<1> A write attempt on key `"key"`.
<2> Another write attempt on key `"key"`.
